        ...
        if not module.check_mode:
            try:
                ...
                if not os.path.exists(v_dest):
                    subprocess.check_output(['sudo', 'mkdir','-p',v_dest])

                if not remote_src and os.path.exists(b_dest) and _original_basename:
                    if not os.path.exists(v_dest):
                        os.makedirs(v_dest)
                    v_dest += b_dest.split('/')[-1] + ".backup." + timestr
                    shutil.copy(b_dest, v_dest)
